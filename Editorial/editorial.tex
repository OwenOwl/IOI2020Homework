...
这一部分仍然可以使用 $\mu$ 函数进行容斥。对于 $x$ 的每个非 $1$ 约数 $d$，再从 $B$ 中减去含这个约数的子集个数，即此部分的答案为：
...
对于求 $cnt_x$，观察发现因 $\mu(x)=0$ 时 $f(x)$ 的值不产生影响，因此只需要求出不含平方因子的数的 $cnt$ 值。可以考虑枚举 $a_i$，将 $a_i$ 分解出 $k$ 个不同质因子，然后 $2^k$ 枚举不含平方因子的因子。
